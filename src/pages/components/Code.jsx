...
const getFileIcon = (filename) => {
  ...
  switch (ext) {
    ...
    case 'json':
      return { icon: FileJson, color: 'text-amber-300' };
    case 'md':
  ...
  lines.forEach((line, index) => {
    ...
    if (text.includes('TODO') || text.includes('FIXME'))
      issues.push({
        ...
        severity: 'warning',
        type: 'Tech Debt',
        message: 'Pending task annotation found.',
      });
    if (text.match(/password|secret|key|token/i) && !text.includes('import'))
      issues.push({
        line: i,
        severity: 'error',
        type: 'Security',
    ...
    if (text.includes('any') && fileName.match(/\.tsx?$/))
      issues.push({
        line: i,
        severity: 'warning',
        type: 'Type Safety',
...
export default function CodeEditorPage() {
  ...
  useEffect(() => {
    const handleMouseMove = (e) => {
      ...
      if (isResizing === 'left')
        ...
        setRightWidth(
          Math.max(250, Math.min(window.innerWidth - e.clientX, 600)),
        );
    };
  ...
  const buildFileTree = async (dirHandle) => {
    ...
    for await (const entry of dirHandle.values()) {
      ...
      if (entry.kind === 'directory')
        tree.children.push(await buildFileTree(entry));
      else
        tree.children.push({ name: entry.name, type: 'file', handle: entry });
    }
  ...
  return (
    <div
      className={`h-screen flex flex-col font-sans overflow-hidden ${THEME.bg} ${THEME.text} ${THEME.selection}`}
    >
      {/* BACKGROUND PATTERN */}
      <div
        ...
        style={{
          backgroundImage:
            "url(\"data:image/svg+xml,%3Csvg width='60' height='60' viewBox='0 0 60 60' xmlns='http://www.w3.org/2000/svg'%3E%3Cg fill='none' fill-rule='evenodd'%3E%3Cg fill='%23525252' fill-opacity='0.4'%3E%3Cpath d='M36 34v-4h-2v4h-4v2h4v4h2v-4h4v-2h-4zm0-30V0h-2v4h-4v2h4v4h2V6h4V4h-4zM6 34v-4H4v4H0v2h4v4h2v-4h4v-2H6zM6 4V0H4v4H0v2h4v4h2V6h4V4H6z'/%3E%3C/g%3E%3C/g%3E%3C/svg%3E\")",
        }}
      ></div>

      {/* HEADER TOOLBAR */}
      <div
        className={`relative z-10 h-14 border-b ${THEME.border} bg-[#09090b]/80 backdrop-blur-md flex items-center justify-between px-6 shrink-0`}
      >
            ...
            {selectedFile ? (
              <div className="flex items-center gap-2 text-zinc-200 bg-zinc-900 px-2 py-1 rounded border border-zinc-800">
                {React.createElement(getFileIcon(selectedFile.name).icon, {
                  size: 12,
                  className: getFileIcon(selectedFile.name).color,
                })}
      ...
      <div className="flex-1 flex overflow-hidden relative z-10">
        ...
        <div
          ...
          className={`flex flex-col border-r ${THEME.border} bg-[#09090b]/50 backdrop-blur-sm`}
        >
        ...
        <div
          ...
          className="w-1 hover:bg-indigo-500 cursor-col-resize z-20 flex items-center justify-center group transition-colors"
        >
          <GripVertical
            size={8}
        ...
        <div className="flex-1 flex flex-col min-w-0 bg-[#0c0c0e] relative">
          ...
          <div
            className={`h-9 flex items-center border-b ${THEME.border} bg-[#09090b]`}
          >
            {selectedFile && (
          ...
          <div className={`flex-1 relative overflow-hidden ${THEME.scroll}`}>
            {selectedFile ? (
              ...
              <div className="h-full flex flex-col items-center justify-center text-zinc-600">
                ...
                <p className="text-sm">
                  Select a file from the explorer to audit
                </p>
              </div>
            )}
        ...
        <div
          ...
          className={`flex flex-col border-l ${THEME.border} bg-[#09090b]/50 backdrop-blur-sm`}
        >
          <div className="h-12 border-b border-zinc-800/50 flex items-center justify-between px-4">
            ...
            <span
              className={`text-[10px] px-2 py-0.5 rounded-full font-bold border ${issues.length > 0 ? 'bg-indigo-500/10 text-indigo-400 border-indigo-500/20' : 'bg-zinc-800 text-zinc-500 border-zinc-700'}`}
            >
            ...
            {selectedFile && !isAnalyzing ? (
              issues.length > 0 ? (
                ...
                <div className="h-64 flex flex-col items-center justify-center text-zinc-600">
                  <CheckCircle2
                    size={32}
                    className="text-emerald-500/50 mb-3"
                  />
      ...
      <div className="h-6 bg-[#09090b] border-t border-zinc-800 flex items-center justify-between px-4 text-[10px] text-zinc-500 select-none z-10">
        <div className="flex items-center gap-4">
          <span className="flex items-center gap-1.5 hover:text-zinc-300 transition-colors">
            <ShieldAlert size={10} /> Security Guard Active
          </span>
          {selectedFile && (
            <span>UTF-8 • {fileContent.split('\n').length} Lines</span>
          )}
        </div>
  ...
  return (
    <div className="select-none">
      <div
        onClick={(e) => {
          e.stopPropagation();
          isFolder ? toggleFolder(node.name) : onFileClick(node);
        }}
        className={`
...
function CodeRenderer({ code, issues, onHoverIssue }) {
  ...
  const issuesMap = issues.reduce(
    (acc, issue) => ({ ...acc, [issue.line]: issue }),
    {},
  );
      ...
      {lines.map((line, idx) => {
        ...
        return (
          <div
            id={`line-${lineNum}`}
            key={idx}
            className={`flex group hover:bg-zinc-800/30 transition-colors ${lineStyle}`}
          >
            <div className="w-10 text-right pr-4 select-none text-xs text-zinc-700 font-medium group-hover:text-zinc-500">
              {lineNum}
            </div>
            <div
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <span
              className={`text-[10px] font-bold uppercase tracking-wider ${styles.color}`}
            >
              {issue.type}
            </span>
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 mb-2 pb-2 border-b border-zinc-800">
        ...
        />
        <span className="font-bold text-xs uppercase text-zinc-300">
          {issue.type} Detected
        </span>
      </div>
      <p className="text-xs text-zinc-400 leading-relaxed">{issue.message}</p>
      <div className="mt-3 flex justify-end">
        <button className="text-[10px] bg-zinc-800 hover:bg-zinc-700 text-white px-2 py-1 rounded transition-colors">
          Quick Fix
        </button>
      </div>
    </motion.div>
